...
- OIDC Well Known endpoint: https://auth1.sandbox.zenith.uk-hub-prod.ozoneapi.co.uk/.well-known/openid-configuration
- baseUrl: https://auth1.sandbox.zenith.uk-hub-prod.ozoneapi.co.uk/

### Step by Step guide to connect to sandbox

#### Step 1: Pre-Requisites for TPP
Ensure that the following pre-requisites are met before onboarding onto Zenith Bank(UK).
...
response = requests.post("https://ob-api1.sandbox.zenith.uk-hub-prod.ozoneapi.co.uk/dynamic-client-registration/v3.1/register",
...
Once a TPP successfully registers to Zenith Bank(UK), they can download the environment file for their client via an Zenith Bank(UK) helper API.

```
Endpoint: https://ob-api1.sandbox.zenith.uk-hub-prod.ozoneapi.co.uk/ozone/v1.0/postman/environment
...
https://rs1.sandbox.zenith.uk-hub-prod.ozoneapi.co.uk
https://auth1.sandbox.zenith.uk-hub-prod.ozoneapi.co.uk/token
...
Once the PSU consent is successful, Zenith Bank(UK) will redirect back to the redirect URI. Copy the Authcode from the URL
